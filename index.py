...
ABI.decode = function (tokenAbi, name, data) {
  var types = this.makeTypeList(tokenAbi, name)
  var ret = []
  if (this.methodID(name, types).toString('hex') !== data.substr(0, 8).toString('hex')) { throw new Error('Method name does not match with data!') }

  data = new Buffer(data.substr(8), 'hex')
  var offset = 0
  for (var i = 0; i < types.length; i++) {
    var type = elementaryName(types[i])
    var parsed = parseType(type, data, offset)
    var decoded = decodeSingle(parsed, data, offset)
    offset += parsed.memoryUsage
    ret.push(new Param(type, decoded))
  }
  return ret
}
